...
function atualizarPolicialCombate(pol,dt,agora,vendoMemorizado=null){
  ...
  // A patrulha já mediu a percepção neste quadro; reaproveita o resultado em combate.
...
function tetoDoNivel(){
  if(policia.alvoPlantacao||policia.alvoPlanta)return Math.min(POLICIAIS_MAX,3);
  if(abordagem.ativa)return Math.min(POLICIAIS_MAX,3);
...
function atualizarPatrulha(dt,agora){
  ...
  for(let i=policiais.length-1;i>=0;i--){
    ...
    if(briga){atualizarPolicialCombate(pol,dt,agora,vendo);continue}
