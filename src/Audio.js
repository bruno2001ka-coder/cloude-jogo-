...
export function tocarSomColisaoCarro(impacto=2,posicao=null){const ctx=obterContexto();if(!ctx)return;if(ctx.state==='suspended'){ctx.resume().then(()=>tocarSomColisaoCarro(impacto,posicao)).catch(()=>{});return}const t=ctx.currentTime,src=ctx.createBufferSource(),f=ctx.createBiquadFilter();src.buffer=ruído(ctx);f.type='lowpass';f.frequency.setValueAtTime(2600,t);f.frequency.exponentialRampToValueAtTime(480,t+.18);const força=Math.min(1,Math.max(.15,impacto/18)),g=ganho(ctx,t,.06+.22*força,.16+.18*força);src.connect(f);f.connect(g);g.connect(ctx.destination);src.start(t);src.stop(t+.22+.18*força)}
